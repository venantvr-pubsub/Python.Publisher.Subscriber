...
class TestMessageHandling:
    ...
    def test_subscription_management(self, in_memory_db):
        ...
        # Check subscriptions
        cursor.execute(
            'SELECT topic FROM subscriptions WHERE consumer = ?',
            ('alice',)
        )
    ...
    def test_consumption_tracking(self, in_memory_db):
        ...
        # Verify consumption
        cursor.execute(
            'SELECT * FROM consumptions WHERE consumer = ? AND message_id = ?',
            ('bob', message_id)
        )
        result = cursor.fetchone()
    ...
    def test_multiple_consumers_same_message(self, in_memory_db):
        ...
        cursor.execute(
            'SELECT COUNT(DISTINCT consumer) FROM consumptions WHERE message_id = ?',
            (message_id,)
        )
    ...
    def test_message_creation_patterns(self, in_memory_db, topic, message):
        ...
        # Retrieve and verify
        cursor.execute(
            'SELECT topic, message, producer FROM messages WHERE topic = ?',
            (topic,)
        )
        result = cursor.fetchone()
